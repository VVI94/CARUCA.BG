...
@WebServlet("/CarImgUploadServlet")
@MultipartConfig(fileSizeThreshold=1024*1024*10, 	 
					   maxFileSize=1024*1024*50,       
					maxRequestSize=1024*1024*100)
public class CarImgUploadServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
    
	private static final String UPLOAD_DIR = "uploads"; 
	
    public CarImgUploadServlet() {
        super();
    } 
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		int carId = Integer.valueOf( (String) request.getSession().getAttribute("carId") );  
		...
		try
		{
			...
			c.createStatement().executeUpdate("UPDATE cars SET photo = '"+filePath.replace('\\', '/')+"' WHERE id = '"+carId+"'" ); 
		}
		catch(Exception e)
		{
